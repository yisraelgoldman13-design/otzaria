...
### משימות
- [ ] 1.1 יצירת פונקציה `scanForChanges(String otzariaPath)` שסורקת את כל התיקיות
- [ ] 1.2 השוואת רשימת הקבצים הקיימים מול הספרים ב-DB
- [ ] 1.3 זיהוי קבצים חדשים (לא קיימים ב-DB)
...
### משימות
- [ ] 2.1 יצירת פונקציה `parsePathToCategories(String filePath, String basePath)`
- [ ] 2.2 הסרת נתיב הבסיס והשארת ההיררכיה בלבד
- [ ] 2.3 הפרדת שם הספר מהקטגוריות
